...
typedef struct weather_response_t {
	...
	bool 		isSuccess;
	// defaults:
	weather_response_t(): gmt(1447058949), temp_now(255), temp_high(255), temp_low(255), conditionCode(-1), isSuccess(false) {};
} weather_response_t;

class Weather {
public:
	Weather(int location, HttpClient* client, String apiKey);
